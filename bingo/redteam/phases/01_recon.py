...
def run(target: str, session: RedTeamSession, on_progress=None) -> list[dict]:
    ...
    for ep in api_found:
        sev = "high" if ep.get("is_json") and ep.get("status") == 200 else "medium"
        if ep.get("status") == 401:
            sev = "info"
        findings.append({
            "type": "api_endpoint",
            "severity": sev,
            "title": f"API 엔드포인트: {ep['path']} [{ep['status']}]"
                     + (" (JSON)" if ep.get("is_json") else ""),
            "detail": f"url={ep['url']} size={ep.get('size',0)} preview={ep.get('preview','')[:100]}",
            "url": ep.get("url", ""),
            "path": ep["path"],
        })
    accessible_api = [e for e in api_found if e.get("status") == 200]
    if accessible_api:
    ...
    # ── Step 8: 약한 비밀번호 브루트포스 (로그인 폼 발견 시) ─────────────
    # 이용자 제보: lahyl:lahy12025 같은 약한 비밀번호 탐지 못함 → 자동 시도
    if login_panels:
        log(f"  [8/8] 약한 비밀번호 브루트포스 ({len(login_panels)}개 로그인 폼)...")
        weak_creds = get_weak_credentials(domain)
        session.metadata["brute_creds_count"] = len(weak_creds)
        all_successes = []

        for panel in login_panels[:3]:  # 최대 3개 패널
            login_url = panel["path"]
            log(f"    → {login_url} 시도 중 ({len(weak_creds)}개 비밀번호)...")
