...
def run(session: RedTeamSession, output_dir: str = ".", on_progress=None) -> str:
    ...
    for ph, pr in session.phases.items():
        ...
        md_lines.append(f"- 발견 건수: {len(pr.findings)}")
        if pr.ai_summary:
            md_lines.append(f"- AI 분석: {pr.ai_summary}")
        md_lines.append("")
...
<h2>취약점 목록</h2>
<table>
  <tr><th>#</th><th>심각도</th><th>유형</th><th>제목</th><th>단계</th></tr>
  {rows}
</table>
</body></html>"""
